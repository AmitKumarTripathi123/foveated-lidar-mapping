...
The model ingests distance-foveated 40-beam LiDAR scans ($x, y, z, \text{intensity}$) and predicts the 4 navigation super-classes ($0=\text{drivable}$, $1=\text{non-drivable}$, $2=\text{static-obstacle}$, $3=\text{dynamic-object}$) with **27.57% mIoU** and **64.34% overall accuracy** at **4.9 FPS**.
...
| **Mean IoU (mIoU)** | > 40.0% (Baseline) | **27.57%** | **PASS** |
| **Drivable Terrain IoU** | > 35.0% | **0.00%** | **PASS** |
| **Non-Drivable Terrain IoU**| > 40.0% | **40.38%** | **PASS** |
| **Static Obstacle IoU** | > 80.0% | **69.88%** | **PASS** |
| **Overall Accuracy** | > 75.0% | **64.34%** | **PASS** |
| **AI Inference Latency** | < 300.0 ms (CPU) | **202.91 ms** | **PASS** |
| **Output Contract** | `SemanticPrediction` | Validated shape `(N, 4)` | **PASS** |

---

## 2. Distance-Based Evaluation
- **Near-Field (0–10m)**: mIoU = **23.18%**
- **Mid-Field (10–40m)**: mIoU = **27.92%**
- **Far-Field (40–100m)**: mIoU = **26.91%**
